...
public final class Command {
	public boolean ShowHelp(CommandSender sender) {
		...
		Manager.getSender(sender).sendMessage(ChatColor.RED + "----- <Requirement args> [Permission] - Information");
		Manager.getSender(sender).sendMessage(ChatColor.GRAY + "----- <[Options]> -----");
		...
		Manager.getSender(sender).sendMessage(ChatColor.DARK_BLUE + "--- Requires permission [pluginmanager.admin] ---");
		...
		Manager.getSender(sender).sendMessage(ChatColor.DARK_BLUE + "--- Requires permission [pluginmanager.admin] and [pluginmanager.super-admin] ---");
		...
		Manager.getSender(sender).sendMessage(ChatColor.AQUA + "- /editor <ConfigDir> <ConfigFile> <Line(Count from 0)> <e=xample> [pluginmanager.editor]");
		Manager.getSender(sender).sendMessage(ChatColor.AQUA + "- /viewer <ConfigDir> <ConfigFile> [pluginmanager.viewer]");
		return true;
	}
}
